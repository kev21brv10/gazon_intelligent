...
def _evaluate_condition_guard(policy: WateringPolicy, weather: dict[str, Any], hydric_state: str | None) -> BlockingEvaluation:
    ...
    if minimum_temperature is not None:
        _temperature = weather.get("temperature_c")
        if _temperature is None:
            # Température inconnue : on bloque, comme le faisait déjà le `except TypeError`
            # déclenché par `float(None)`. Rendu explicite plutôt que confié à l'exception.
            return BlockingEvaluation(True, "temperature_unknown")
        try:
            if float(_temperature) < float(minimum_temperature):
